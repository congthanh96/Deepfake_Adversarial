...
if __name__ == "__main__":
    ...
    target_model.eval()
    print('=================== cifar10_vgg19_bn ===============================================')
    # load the generator of adversarial examples
    #pretrained_generator_path = './models/netG.pth.tar.38'
    #pretrained_G = models.Generator(gen_input_nc, image_nc).to(device)
    #pretrained_G.load_state_dict(torch.load(pretrained_generator_path))
    #pretrained_G.eval()
    pretrained_generator_path = 'C:/Users/nguye/OneDrive/Learning/ThacSiATTTKMA/LuanVan/CodeBackup/vgg19bn/models/netG.pth.tar.600'
    ...
    # test adversarial examples in cifar10 testing dataset
        ...
        for i, data in enumerate(test_dataloader, 0):
            ...
            cv2.imwrite(f"./preview/cifar10_vgg19_bn/0_{kcount}/id_{count}-0origin -label_{test_label[0].detach().cpu().numpy()}-classname_{CLASS_NAMES[test_label[0].detach().cpu().numpy()]}.jpg",
                                test_img[0].permute((1,2,0)).detach().cpu().numpy()*255)

            cv2.imwrite(f"./preview/cifar10_vgg19_bn/0_{kcount}/id_{count}-1targerted-label_{targeted_lb[0].detach().cpu().numpy()}_{targeted_score}-classname_{CLASS_NAMES[targeted_lb[0].detach().cpu().numpy()]}.jpg",
                                test_img[0].permute((1,2,0)).detach().cpu().numpy()*255)

            cv2.imwrite(f"./preview/cifar10_vgg19_bn/0_{kcount}/id_{count}-2pert.jpg",
                                perturbation[0].permute((1,2,0)).detach().cpu().numpy()*255)

            cv2.imwrite(f"./preview/cifar10_vgg19_bn/0_{kcount}/id_{count}-3adv-label_{pred_lab[0].detach().cpu().numpy()}_{pred_scores}-classname_{CLASS_NAMES[pred_lab[0].detach().cpu().numpy()]}.jpg",
                                adv_img[0].permute((1,2,0)).detach().cpu().numpy()*255)
            image1 = io.imread(f"./preview/cifar10_vgg19_bn/0_{kcount}/id_{count}-0origin -label_{test_label[0].detach().cpu().numpy()}-classname_{CLASS_NAMES[test_label[0].detach().cpu().numpy()]}.jpg")
            image2 = io.imread(f"./preview/cifar10_vgg19_bn/0_{kcount}/id_{count}-3adv-label_{pred_lab[0].detach().cpu().numpy()}_{pred_scores}-classname_{CLASS_NAMES[pred_lab[0].detach().cpu().numpy()]}.jpg")
