...
if __name__ == "__main__":
    ...
    #pretrained_G.eval()
    pretrained_generator_path = './models/netG.pth.tar.500'
    ...
    for kcount in range(1,11):
        ...
        for i, data in enumerate(test_dataloader, 0):
            ...
            cv2.imwrite(f"./preview/cifar10_repvgg_a2/0_{kcount}/id_{count}-0origin -label_{test_label[0].numpy()}-classname_{CLASS_NAMES[test_label[0].numpy()]}.jpg",
                                test_img[0].permute((1,2,0)).numpy()*255)

            cv2.imwrite(f"./preview/cifar10_repvgg_a2/0_{kcount}/id_{count}-1targerted-label_{targeted_lb[0].numpy()}_{targeted_score}-classname_{CLASS_NAMES[targeted_lb[0].numpy()]}.jpg",
                                test_img[0].permute((1,2,0)).numpy()*255)

            cv2.imwrite(f"./preview/cifar10_repvgg_a2/0_{kcount}/id_{count}-2pert.jpg",
                                perturbation[0].permute((1,2,0)).detach().numpy()*255)

            cv2.imwrite(f"./preview/cifar10_repvgg_a2/0_{kcount}/id_{count}-3adv-label_{pred_lab[0].numpy()}_{pred_scores}-classname_{CLASS_NAMES[pred_lab[0].numpy()]}.jpg",
